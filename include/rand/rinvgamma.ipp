/*################################################################################
  ##
  ##   Copyright (C) 2011-2018 Keith O'Hara
  ##
  ##   This file is part of the StatsLib C++ library.
  ##
  ##   StatsLib is free software: you can redistribute it and/or modify
  ##   it under the terms of the GNU General Public License as published by
  ##   the Free Software Foundation, either version 2 of the License, or
  ##   (at your option) any later version.
  ##
  ##   StatsLib is distributed in the hope that it will be useful,
  ##   but WITHOUT ANY WARRANTY; without even the implied warranty of
  ##   MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
  ##   GNU General Public License for more details.
  ##
  ################################################################################*/

/* 
 * Sample from an inverse-gamma distribution
 */

template<typename T>
T
rinvgamma(T shape_par, T rate_par)
{
    return T(1.0) / rgamma(shape_par,T(1.0)/rate_par);
}

#ifndef STATS_NO_ARMA

template<typename Ta, typename Tb>
arma::Mat<Tb>
rinvgamma(const uint_t n, const Ta shape_par, const Ta rate_par)
{
    return rinvgamma<Ta,Tb>(n,1U,shape_par,rate_par);
}

template<typename Ta, typename Tb>
arma::Mat<Tb>
rinvgamma(const uint_t n, const uint_t k, const Ta shape_par, const Ta rate_par)
{
    arma::Mat<Tb> ret(n,k);
    
    //

    Tb* ret_mem = ret.memptr();

#ifndef STATS_NO_OMP
    #pragma omp parallel for
#endif
    for (uint_t j=0U; j < n*k; j++)
    {
        ret_mem[j] = rinvgamma(shape_par,rate_par);
    }

    //
    
    return ret;
}

#endif
