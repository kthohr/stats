...
template<typename T>
statslib_inline
return_t<T> rnorm(const T mu_par, const T sigma_par, ullint_t seed_val = std::random_device{}());
...
template<typename T>
statslib_inline
void rnorm_int(const T mu_par, const T sigma_par, T* vals_out, const ullint_t num_elem);
